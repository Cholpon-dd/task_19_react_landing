import React, {useState} from 'react'
import { Link, NavLink } from 'react-router-dom'
import './navbar.scss'
import menuIcon from '../../images/menu-thin.svg'


const Navbar = () => {

    const [menuActive, setMenuActive] = useState(false)

    const showMenu = () => {
        setMenuActive(!menuActive)
    }

  return (
    <nav className='navbar'>
        <div className="container navbar__container">
            <Link to="/" className='navbar__logo'>WorkSpace</Link>
            <button onClick={showMenu} className='navbar__toggle'>
                <img src={menuIcon} alt="menu icon" />
            </button>
            <div className={menuActive ? "navbar__menu navbar__menu--show" : "navbar__menu"}>
                <NavLink onClick={showMenu} to="/" className="navbar__menu-link">Home</NavLink>
                <NavLink onClick={showMenu} to="/about" className="navbar__menu-link">About</NavLink>
                <NavLink onClick={showMenu} to="/workspace" className="navbar__menu-link">Workspace</NavLink>
                <NavLink onClick={showMenu} to="/price" className="navbar__menu-link">Pricing</NavLink>
                <NavLink onClick={showMenu} to="/contact" className="navbar__menu-link">Contacts</NavLink>
            </div>
        </div>
    </nav>
  )
}

export default Navbar